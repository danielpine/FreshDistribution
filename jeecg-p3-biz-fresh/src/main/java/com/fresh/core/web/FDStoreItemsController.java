...
public class FDStoreItemsController extends BaseController {
  ...
  public void list(@ModelAttribute FDStoreItemsEntity query, HttpServletRequest request,
  ...
  public void fDStoreItemsDetail(@RequestParam(required = true, value = "id") String id,
  ...
  @ResponseBody
  public AjaxJson findGrid(@ModelAttribute DateVo dateVo, @ModelAttribute FDGoodsEntity goods,
      @ModelAttribute FDStoreEntity store, @ModelAttribute FDStoreItemsEntity fDStoreItems) {
    AjaxJson j = new AjaxJson();
    LOG.info("{}", dateVo);
    LOG.info("{}", goods);
    LOG.info("{}", store);
    ...
    try {
      List<StoreVo> list = fDQueryDao.getAll(goods, dateVo, store);
      j.setObj(list);
      j.setMsg("保存成功");
    } catch (Exception e) {
      j.setSuccess(false);
